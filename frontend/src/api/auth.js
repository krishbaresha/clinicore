...
export async function login(identifier, password) {
  ...
  const session = {
    ...
    is_owner: Boolean(user.is_owner || user.role === "owner"),
    can_view_financials: Boolean(user.can_view_financials || user.role === "admin" || user.role === "owner" || user.is_owner),
...
export function getSession() {
  try {
    ...
    const validated = {
      ...
      is_owner: Boolean(dbUser.is_owner || dbUser.role === "owner"),
      can_view_financials: Boolean(dbUser.can_view_financials || dbUser.role === "admin" || dbUser.role === "owner" || dbUser.is_owner),
