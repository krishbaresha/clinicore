...
export default function LoginScreen() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between p-6 sm:p-12 bg-white relative">
        <header className="flex items-center justify-between w-full flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight text-teal-950 leading-none">H/Dr. Asif Ashraf Clinic</h1>
              <p className="text-[10px] text-teal-700 font-black uppercase tracking-wider mt-1">Clinical OS</p>
            </div>
          </div>
        </header>
          ...
          <main className="w-full max-w-lg mx-auto my-auto py-6">
            ...
            <div className="mt-8 pt-4 border-t border-slate-100 flex items-center justify-center">
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-slate-50 border border-slate-200 hover:bg-slate-100 font-extrabold text-xs text-slate-700 transition-all cursor-pointer active:scale-95"
              >
              ...
              {usersList.length > 0 ? (
                usersList.map((u) => (
                  <button
                    ...
                    className="w-full bg-white border border-slate-200 hover:border-teal-600/80 rounded-2xl p-4 flex items-center justify-between transition-all active:scale-[0.99] cursor-pointer shadow-xs"
                  >
        ...
        {view === "PIN_ENTRY" && (
          <main className="w-full max-w-xs mx-auto my-auto py-6">
            <div className="text-center mb-4">
              <button
                onClick={() => setView("STAFF_SELECT")}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-slate-600 text-xs font-bold transition-all cursor-pointer mb-3"
              >
              ...
              <div className="relative w-16 h-16 mx-auto mb-2">
                <div className="w-16 h-16 rounded-2xl bg-teal-50 border border-teal-100 text-teal-800 flex items-center justify-center font-black text-xl uppercase">
            ...
            <div className="flex items-center justify-center gap-2 mb-4 h-6">
              {pinInput.split("").map((char, idx) => (
                <div key={idx} className="w-3.5 h-3.5 rounded-full bg-teal-600 scale-110 shadow-xs" />
              ))}
              {pinInput.length === 0 && <span className="text-[11px] text-slate-400 font-semibold tracking-wider">Enter login PIN</span>}
            </div>

            {error && (
              <div className="p-3 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 text-xs font-bold text-center mb-4">
                {error}
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                <button
                  ...
                  className="w-16 h-16 rounded-full border border-slate-200 bg-white hover:bg-slate-50 active:scale-95 font-bold text-xl text-slate-900 transition-all cursor-pointer mx-auto flex items-center justify-center shadow-2xs"
                >
                  {num}
                </button>
              ))}
              <button
                ...
                className="w-16 h-16 rounded-full border border-slate-200 bg-slate-50 hover:bg-slate-100 active:scale-95 font-bold text-xs text-rose-600 transition-all cursor-pointer mx-auto flex items-center justify-center"
              >
                Clear
              </button>
              <button
              ...
              <button
                ...
                className="w-16 h-16 rounded-full border border-slate-200 bg-slate-50 hover:bg-slate-100 active:scale-95 text-slate-600 transition-all cursor-pointer mx-auto flex items-center justify-center"
              >
            ...
            <button
              ...
              className="w-full min-h-[46px] bg-gradient-to-r from-teal-700 to-teal-600 hover:from-teal-800 hover:to-teal-700 text-white font-extrabold py-3 rounded-2xl text-xs uppercase tracking-wider transition-all shadow-md shadow-teal-700/20 flex items-center justify-center gap-2 cursor-pointer active:scale-95 disabled:opacity-50"
            >
